...
function walker (path, options) {
	...
	return Fsp.readdir(path).then(function (files) {
		...
		var promises = files.map(function (file) {
			...
			if (options.ignoreDot && file.startsWith('.')) {
				return Promise.resolve(null);
			}

			if (options.filters.length > 0 && options.filtersRegExp.test(file)) {
				return Promise.resolve(null);
			}

			return Fsp.stat(Path.join(path, file)).then(function(stats) {
				...
				if (stats.isDirectory()) {
					...
					return Promise.resolve(Path.join(path, file));
				}
